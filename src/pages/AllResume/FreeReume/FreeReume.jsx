...
const FreeReume = () => {
    ...
    return (
        <div>
            <div className="my-20">
                <h2 className="font-bold text-3xl md:text-5xl"><span className="text-[#42C3E4]">{t('freeResume.title1')}</span> {t('freeResume.title2')}</h2>
                <p className="text-xl">{t('freeResume.description')}</p>
                ...
                <div className="grid gird-cols sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 my-10">
                    {visibleTemplates.map((resume, index) => (
                        <div key={index} className="p-5 resumeContents relative bg-slate-100">
                            ...
                            <button className='useButton'>{t('freeResume.useTemplate')}</button>
                        </div>
                    ))}
                </div>
